...
Towns = read_csv("D:/DataScienceAssignment/Cleaned datasets/Cleaned_Town_population.csv")%>% 
  select(shortPostcode, Town, District, County)
BroadbandCleaned = read_csv("D:/DataScienceAssignment/cleaned datasets/Cleaned_Broadband_Speed.csv", show_col_types = FALSE)
...
Towns_Population = read_csv("D:/DataScienceAssignment/Cleaned datasets/Cleaned_Town_population.csv") %>% 
  select(-Year) 
HousePrices=read_csv("D:/DataScienceAssignment/Cleaned datasets/Cleaned_House_Pricing_2019-2022.csv", show_col_types = FALSE)
...
write.csv(House_town,"D:/DataScienceAssignment/cleaned datasets/HOUSE_TOWN.csv",row.names = FALSE)
...
Town = read_csv("D:/DataScienceAssignment/Cleaned datasets/Cleaned_Town_population.csv")%>% 
  select(-Year)
crime_Data = read_csv("D:/DataScienceAssignment/Cleaned datasets/Cleaned_Crime_Data.csv")
...
RobberyData %>% 
  ...
  geom_col(color ="blue") +
  geom_label(aes(label = labels),color="green",
...
radar_chart <- radarchart(as.data.frame(pivot_data)[-1, ],
                          axistype = 1,
                          seg = 4,
                          plty = 1:2,
                          pcol = c("blue", "red"),
...
# Step 6: Modify the radar chart parameters
radar_chart$title <- "Vehicle Crime Rate 2021-2022"

# Step 7: Print the radar chart
print(radar_chart)
...
schoolData = read_csv('D:/DataScienceAssignment/Cleaned Datasets/Cleaned_School_Data.csv', show_col_types = FALSE)
OXFORDSHIREschool = read_csv("D:/DataScienceAssignment/Cleaned Datasets/OXFORSSHIRESchoolData.csv")
YORKSHIREschool = read_csv('D:/DataScienceAssignment/Cleaned Datasets/YORKSHIRESchoolData.csv')
...
prices = read_csv("D:/DataScienceAssignment/cleaned datasets/Cleaned_House_Pricing_2019-2022.csv")

speeds = read_csv("D:/DataScienceAssignment/Cleaned datasets/Cleaned_Broadband_Speed.csv") %>% 
  na.omit()  

crime=read_csv("D:/DataScienceAssignment/cleaned datasets/Cleaned_Crime_Data.csv")

schools=read_csv("D:/DataScienceAssignment/Cleaned Datasets/Cleaned_School_Data.csv") %>% 
  na.omit()
...
color= c("OXFORDSHIRE" = "purple", "YORKSHIRE" = "brown")
...
Towns_Populations = read_csv("D:/DataScienceAssignment/Cleaned datasets/Cleaned_Town_population.csv")
...
color= c("OXFORDSHIRE" = "red", "YORKSHIRE" = "green")
...
color <- c("OXFORDSHIRE" = "brown", "YORKSHIRE" = "purple")
